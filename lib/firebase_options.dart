...
class FirestoreService {
  ...
  Stream<QuerySnapshot> getPrivateFeed(List<String> followingIds) {
    print('getPrivateFeed called with ${followingIds.length} following IDs');
    
    if (followingIds.isEmpty) {
      print('Following list is empty, returning empty stream');
      // Return empty stream if not following anyone
      return Stream.empty();
    }
    
    // Firestore 'whereIn' has a limit of 10 items
    // If following more than 10, take first 10
    final limitedIds = followingIds.length > 10 
        ? followingIds.sublist(0, 10) 
        : followingIds;
    
    print('Fetching posts from users: $limitedIds');
